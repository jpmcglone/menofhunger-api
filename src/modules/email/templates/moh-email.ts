...
export const EMAIL_CLASS = {
  ...
  border: "moh-border",
  rule: "moh-rule",
  monochromeButton: "moh-button-monochrome",
} as const;
...
function darkModeColorRules(): EmailColorRule[] {
  ...
  return [
    ...
    {
      selectors: [`.${EMAIL_CLASS.surface}`],
      decls: `background-color:${EMAIL_DARK.surface}!important;`,
    },
    {
      selectors: [`.${EMAIL_CLASS.elev}`],
      decls: `background-color:${EMAIL_DARK.elevated}!important;`,
    },
    {
      selectors: [
        `.${EMAIL_CLASS.bg} *`,
    ...
    },
    {
      selectors: [
        `.${EMAIL_CLASS.muted}`,
        `.${EMAIL_CLASS.bg} .${EMAIL_CLASS.muted}`,
        `.${EMAIL_CLASS.surface} .${EMAIL_CLASS.muted}`,
      ],
      decls: muted,
    },
    {
      selectors: [
        `.${EMAIL_CLASS.soft}`,
        `.${EMAIL_CLASS.bg} .${EMAIL_CLASS.soft}`,
        `.${EMAIL_CLASS.surface} .${EMAIL_CLASS.soft}`,
      ],
      decls: soft,
    },
    {
      selectors: [
        `.${EMAIL_CLASS.link}`,
        `.${EMAIL_CLASS.bg} .${EMAIL_CLASS.link}`,
        `.${EMAIL_CLASS.surface} .${EMAIL_CLASS.link}`,
      ],
      decls: link,
    },
    {
      selectors: [
        `.${EMAIL_CLASS.monochromeButton}`,
        `.${EMAIL_CLASS.bg} .${EMAIL_CLASS.monochromeButton}`,
        `.${EMAIL_CLASS.surface} .${EMAIL_CLASS.monochromeButton}`,
      ],
      decls: `background-color:${EMAIL_DARK.text}!important;color:${EMAIL_DARK.page}!important;`,
    },
    {
      selectors: [`.${EMAIL_CLASS.border}`],
      decls: `border-color:${EMAIL_DARK.border}!important;`,
    },
    {
      selectors: [`.${EMAIL_CLASS.rule}`],
      decls: `border-top-color:${EMAIL_DARK.border}!important;`,
    },
  ];
}
...
export function renderPill(
  ...
  toneOrOptions:
    ...
    | {
        tone?: PillTone;
        actorTier?: PillActorTier | null;
        postVisibility?: PillPostVisibility | null;
      } = "neutral",
): string {
  const opts =
  ...
  const semantic =
    (opts.actorTier ?? null) === "premium" ||
    (opts.postVisibility ?? null) === "premiumOnly"
      ? { bg: "#F3E6C8", border: EMAIL.brass, text: EMAIL.brass }
      : (opts.actorTier ?? null) === "verified" ||
          (opts.postVisibility ?? null) === "verifiedOnly"
        ...
        : (opts.actorTier ?? null) === "organization"
          ...
          : (opts.actorTier ?? null) === "onlyMe" ||
              (opts.postVisibility ?? null) === "onlyMe"
...
export function renderButton(params: {
  ...
  const style =
    variant === "secondary"
      ...
      : variant === "monochrome"
        ? `display:inline-block;padding:10px 14px;border-radius:10px;background:${EMAIL.text};color:${EMAIL.page};text-decoration:none;font-weight:800;font-size:13px;`
  ...
  const sizeStyle =
    params.size === "large" ? `padding:12px 16px;line-height:24px;` : "";
  const cls =
    variant === "secondary"
      ? ` class="${EMAIL_CLASS.elev} ${EMAIL_CLASS.text} ${EMAIL_CLASS.border}"`
      : variant === "monochrome"
        ? ` class="${EMAIL_CLASS.monochromeButton}"`
        : "";
  return `<a href="${href}"${cls} style="${style}${sizeStyle}">${label}</a>`;
}
...
export function renderMohEmail(params: {
  ...
  /** Optional site navigation for newsletters; transactional shells stay unchanged. */
  brandHref?: string;
}): string {
  const title = escapeHtml(params.title);
  const preheader = escapeHtml(params.preheader);
  const footer = (params.footerHtml ?? "").trim();
  const brand = params.brandHref
    ? `<a href="${escapeHtml(params.brandHref)}" class="${EMAIL_CLASS.muted}" style="color:${EMAIL.muted};text-decoration:none;">Men of Hunger</a>`
    : "Men of Hunger";
  ...
  return [
    ...
    `<div class="${EMAIL_CLASS.muted}" style="font-size:11px;font-weight:800;letter-spacing:0.12em;text-transform:uppercase;color:${EMAIL.muted};">${brand}</div>`,
    `</td></tr>`,
