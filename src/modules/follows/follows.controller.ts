...
export class FollowsController {
  ...
  async recommendations(@CurrentUserId() viewerUserId: string, @Query() query: unknown) {
    ...
    if (interestKeys && interestKeys.length > 0) {
      const result = await this.follows.recommendArenaUsersToFollow({
        viewerUserId,
        interestKeys,
        limit,
        seed: parsed.seed,
    ...
    const result = await this.follows.recommendUsersToFollow({ viewerUserId, limit, seed: parsed.seed });
    return { data: result.users };
  }
  ...
  @Get('top-users')
  async topUsers(@OptionalCurrentUserId() userId: string | undefined, @Query() query: unknown) {
    const parsed = topUsersSchema.parse(query);
    const limit = parsed.limit ?? 12;
    const result = await this.follows.listTopUsers({ viewerUserId: userId ?? null, limit });
    return { data: result.users };
  }
