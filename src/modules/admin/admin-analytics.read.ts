...
export async function readAdminAnalytics(
  ...
  const byStatus = Object.fromEntries(
    monetizationByStatusRaw.map((r) => [r.stripe_status, Number(r.cnt)]),
  );
  const postsByVisibility = Object.fromEntries(
    postVisibilityRaw.map((r) => [r.visibility, Number(r.cnt)]),
  );
  ...
  const articles: AdminAnalyticsArticlesDto = {
    ...
    byVisibility: Object.fromEntries(
      articleVisibilityRaw.map((r) => [r.visibility, Number(r.cnt)]),
    ),
  ...
  const coins: AdminAnalyticsCoinsDto = {
    ...
    mintedByMultiplier: Object.fromEntries(
      coinsMintedByMultiplierRaw.map((r) => [String(r.amount), Number(r.cnt)]),
    ),
    velocityRatio,
    giniCoefficient:
